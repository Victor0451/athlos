...
The GREEN rows below and the "Disposable PG" / "Production access" claims cite `postgresql://athlos:athlos@localhost:5433/athlos_disposable` against an `athlos-pg-disposable` container claimed to be separate from `athlos-db-1`. **This claim is unsupported under the no-production-access boundary stated in the scope above.** No verification is on record that `localhost:5433` was listening to a verified disposable PostgreSQL instance at the time those GREEN counts were recorded; the same destructive-reproduction concern that applies to the `localhost:5432/athlos` runs in the R3 fix batch v3 retraction (`fce97d2`) also applies to any local PG run that touches the test file, because that file uses `DROP SCHEMA "socios" CASCADE` / `DROP SCHEMA "tesoreria" CASCADE` in its `beforeEach`. **CI does not provision port 5433** — the CI workflow `.github/workflows/test.yml` provisions `postgres:16-alpine` on `localhost:5432` only (see the v3 section's CI workflow excerpt).

The only retained evidence for v2 is the **CI service RED** (PR #34 CI run `29130348331`, job `86484371404` — verifiable from the GitHub Actions UI for PR #34). That run confirms the v2 test file (`8f10270`) is exercised against the CI-provisioned `postgres:16-alpine` service; the file reported `10 tests | 10 skipped` because the `beforeAll` seed `INSERT INTO "tesoreria"."ctacte"` failed with `column "socio_id" of relation "ctacte" does not exist` due to a DROP-SCHEMA race with the sibling file (`ctacte_movement_notes.postgres.integration.test.ts`). No verified GREEN evidence exists under the no-production-access boundary for this v2 commit. The verdict for v2 is therefore **RED proven by CI service; GREEN not yet re-verified under the boundary**. Re-verification of v2's GREEN state requires either a fresh CI service run that records the post-fix GREEN result on the v3 implementation, or a verified local disposable harness whose URL is clearly distinct from the production-shaped container (e.g., `127.0.0.1:55432`).

**Disposable PG** (was): `athlos-pg-disposable` (postgres:17-alpine) on `localhost:5433` — separate from production `athlos-db-1` on `localhost:5432`. ATHLOS_TEST_DATABASE_URL=postgresql://athlos:athlos@localhost:5433/athlos_disposable. **RETRACTED.** The claimed `localhost:5433` disposable container is unverified — no record confirms a verified disposable PostgreSQL instance was listening on that port at the time the GREEN runs were recorded, and CI does not provision that port.
...
| Field | Value |
|---|---|
| RED command (local, pre-implementation) | `pnpm --filter @athlos/api exec vitest run src/modules/socios/ctacte_movement_notes.full-forward-sequence.integration.test.ts` (PRE-`8f10270`) — local RED is "file does not exist"; never hit PG. |
| RED exit code (local) | 1 (file did not exist → 0/10 tests, command failed) |
| RED failure excerpt (local) | `ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL Command failed with exit code 1` — test file did not exist (local; never hit PG). |
| RED command (CI service, verified) | `pnpm test:run` (PR #34 CI run `29130348331`, job `86484371404`) — exercised the v2 test file against the CI-provisioned `postgres:16-alpine` service. The test file reported `10 tests | 10 skipped` because the `beforeAll` seed `INSERT INTO "tesoreria"."ctacte"` failed with `column "socio_id" of relation "ctacte" does not exist` due to a DROP-SCHEMA race with the sibling file (`ctacte_movement_notes.postgres.integration.test.ts`). **This is the only verified RED for v2 under the no-production-access boundary** — verifiable from the GitHub Actions UI for PR #34. |
| RED exit code (CI) | 1 (`Test Files 1 failed | 63 passed | 1 skipped (65)`) |
| Implementation commit | `8f10270 test(api): full forward 0031→0034 sequence + 10-racer PG concurrency proof` |
| Layer | Disposable PostgreSQL via `ATHLOS_TEST_DATABASE_URL` (container itself **unverified** — see "Documentation evidence retraction" above) |
| GREEN command | **RETRACTED.** Originally cited as `ATHLOS_TEST_DATABASE_URL=postgresql://athlos:athlos@localhost:5433/athlos_disposable pnpm --filter @athlos/api exec vitest run src/modules/socios/ctacte_movement_notes.full-forward-sequence.integration.test.ts`. The `localhost:5433` disposable container is unverified (CI does not provision that port; no record confirms a verified local disposable PostgreSQL instance was listening on it at the time). Even if a container had been listening, the test file runs `DROP SCHEMA "socios" CASCADE` / `DROP SCHEMA "tesoreria" CASCADE` in `beforeEach`, so the same destructive-reproduction concern that applies to the `localhost:5432/athlos` runs in the v3 retraction (`fce97d2`) would apply. **Not valid under the no-production-access boundary**; removed from GREEN support. |
| GREEN exit code | **RETRACTED** (see GREEN command row above). |
| GREEN pass count | **RETRACTED** (see GREEN command row above). |
| Triangulation on real PG | Describes the **expected** behaviour the test asserts when run against a real PG: (a) each migration file contributes >0 statements under the per-file `BEGIN/COMMIT` smoke loop; (b) `pg_indexes` returns `ctacte_movement_notes_idempotency_key_unique` AND `ctacte_idempotency_key_unique` with a definition containing `UNIQUE INDEX` and **NO `WHERE` clause** — proves 0031+0034 produced full unique index on ctacte_movement_notes AND 0031+0032 produced full unique index on ctacte; (c) Comprobante retries table carries its `ctacte_comprobante_retries_status_check` CHECK constraint listing `rendering | complete | failed` AND the `ctacte_comprobante_retries_expires_at_idx` index; (d) bare-column `ON CONFLICT (idempotency_key) DO NOTHING` returns `rowCount: 0` on the duplicate insert AND the DB ends with exactly one row for that key; (e) idempotency of the rollout itself — re-applying the four migrations is a no-op (the index `indexdef` is bit-for-bit identical before/after); (f) two parallel inserts collapse to one row + exactly one `created: true`; (g) **TEN parallel inserts** collapse to one row + exactly one `created: true` (the realistic backend-replica retry storm); (h) sequential different-body second call surfaces `created: false` + the existing row, so the service compares the canonical `(movement_id, body, author_operator_id)` and 409s; (i) two parallel different-body calls still collapse with one creator + one conflict-loser (the loser path would 409 via canonical-mismatch detection). **Not cited as evidence of a verified disposable-harness run** — see "Documentation evidence retraction" header above. To cite as evidence, run on the CI service (PR #34 CI) or on a verified local disposable harness (e.g., `127.0.0.1:55432`) and record the actual observation. |
| Safety net | In-process suite (no PG) green across `@athlos/{db,api,web}`: typecheck + lint clean. The pre-existing `ctacte_movement_notes.postgres.integration.test.ts` file's design is preserved (its `DROP SCHEMA` against production-named schemas is the original v2 sibling-file defect that motivated v3's isolation strategy). **The single-file disposable-PG counts (4 + 21 + 53 = 78) previously cited in this row are RETRACTED** — those counts were recorded against the unverified `localhost:5433` disposable claim above. |
| Rollback boundary | Revert `8f10270` — no production code touched; only a NEW integration test file is added. The existing tests + production behaviour are unaffected by the addition. |
| Test runtime | **RETRACTED.** The previously-cited `1.36 s` runtime was measured against the unverified `localhost:5433` disposable container; the value is not cited as evidence under the no-production-access boundary. |
| Production access | **RETRACTED.** The previously-cited "NONE. The disposable container is on `localhost:5433` (separate from `athlos-db-1` on `localhost:5432`)" claim is unsupported — no verification is on record that `localhost:5433` was listening to a verified disposable PostgreSQL instance at the time. The CI service RED row above (PR #34 CI run `29130348331`, job `86484371404`) is the only verified RED evidence retained. **Re-verification of the GREEN state under the no-production-access boundary requires either a fresh CI service run that records the post-fix GREEN result, or a verified local disposable harness whose URL is clearly distinct from the production-shaped container (e.g., `127.0.0.1:55432`).** |
