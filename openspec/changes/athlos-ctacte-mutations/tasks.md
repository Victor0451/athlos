...
- [x] R2.5 — Close Strict-TDD evidence in `apply-progress.md`: for R2.1–R2.4 record cited pre-change commit, exact RED command/exit/failure, implementation commit, exact GREEN command/exit/pass count, triangulation, and safety net; explicitly mark unavailable RED evidence missing. (R2b branch `fix/ctacte-mutations-r2b`; disposable-PostgreSQL evidence captured against `postgresql://athlos:athlos@localhost:55433/athlos`; RED rows that cannot be cited today are explicitly reported as `MISSING` per task description.)
