...
## Unit 5B1 — Atomic MANUAL tender + manual-source writer (service layer)

- Scope: service-layer atomicity only; routes/DTO/permissions unchanged (residual U5-B2). `tasks.md` combined U5-B checkboxes stay `[ ]`; split documented in tasks.md note.
- Behavior: MANUAL branch validates attribution before any DB access — accountCode and description are required together (incomplete → `VALIDATION_ERROR`; blank counts as absent, preserving the legacy no-attribution flow byte-for-byte). Inside the same transaction: idempotent replay check → shift lock/policy → tender INSERT → `resolveAccountSnapshot` (recursive CTE over `contabilidad.plan_cuentas`, active+imputable required, else `CONFLICT 'Manual movement account is unavailable'`) → `dues_cash_manual_sources` INSERT with immutable code/name/path snapshots (retained-row `FOR UPDATE` fallback) → audit with `manualSourceId`. Tender+source+audit commit or roll back together; no amount/method duplication.
- Retroactive-RED note: two writer runs timed out mid-task; the first implemented before testing (ordering deviation acknowledged). RED evidence captured retroactively: with implementation stashed, `tsc --noEmit` failed with 16+ TS2353/TS2339 errors (`accountCode` absent from `TenderCommand`); behavioral RED lives in the PG suite (DB-guard cases fail without the writer).
- Verification (final counts after churn reduction):
  - `pnpm --filter @athlos/api exec vitest run src/modules/dues/cash-desk.test.ts src/modules/dues/cash-desk-transaction.test.ts` — 23 passed; full non-PG dues suite `vitest run src/modules/dues` (PG excluded) — 28 files, 193 passed, 0 failed, 0 skipped (includes 3 behavioral stub-db attribution tests).
  - `scripts/lib/disposable-postgres.sh run --caller caja-u5b1-final -- pnpm --filter @athlos/api exec vitest run src/modules/dues/manual-source-writer.postgres.integration.test.ts src/modules/dues/cash-sources.postgres.integration.test.ts src/modules/dues/cash-desk.postgres.integration.test.ts` — 3 files, 27 passed, 0 failed, 0 skipped; lifecycle `1789492537-1612121-3b717d1bdd07cc39` container+volume removed, absence confirmed.
  - `tsc --noEmit`, eslint, prettier --check on touched files — PASS.
- PG coverage: happy-path snapshots (code/name/path exact); inactive and non-imputable accounts → CONFLICT with zero tender/source rows (atomic rollback); replay same caller_key → persisted tender, exactly one source; BANK_DEBIT expense per matrix; ADMIN (`role='A'`) cross-shift allowed; non-owner finance user denied by DB guard.
- Churn: peak candidate 478 additions (prettier-inflated drafts) reduced to final ≤400 measured by `git diff --stat` at settle time; implementation cash-desk.ts +73, unit tests +48, PG suite 251, docs the remainder.
