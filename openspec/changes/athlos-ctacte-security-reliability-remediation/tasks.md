...
- **S0/PR 1** specs+0034 lifecycle. Test `scripts/check-spec-deltas.mjs`+`0034.lifecycle.test.ts` (disposable PG). Rollback: `openspec/specs/*`.
...
- [ ] 5.2 GREEN: bound owner/follower wait to 30s; mark `failed`; emit `RENDER_TIMEOUT` log
