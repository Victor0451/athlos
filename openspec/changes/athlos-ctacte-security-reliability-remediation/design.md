...
| Heartbeat-only lease vs bounded actor lease | Deadline may discard slow work | Include `operatorId` in the request fingerprint and persisted ownership check. Bound owner and follower wait to 30s; owner-conditional failure marks `failed`, emits structured failure telemetry, and the route returns `504 { error: 'GATEWAY_TIMEOUT' }`. Same actor/key/payload replays stored bytes; another actor or payload gets `409`. |
