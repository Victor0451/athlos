...
- [x] TASK-052 — **PR 7 — packages/import — DBF reader** — `packages/import/src/dbf-reader.ts` using `xbase` (or `dbaf`) to read 14 tables from `LEGACY_DBF_PATH`. Export `readTable(name: TableName): AsyncIterable<LegacyRecord>` with `TableName` enum matching the 14 ordered tables from proposal §Approach. **Files (new)**: 1. **Lines ~100. **Deps: PR 1. **AC**: reading `paramet.DBF` returns ~50 rows with all 12 columns; reading non-existent path throws `LEGACY_UNAVAILABLE`. **Note**: Implemented in PR 7a (commit 59153bf, merged 597b94d). Uses `dbf-reader` (the design mentioned `xbase` but that package is not on the public npm registry; `dbf-reader` 1.0.3 is the maintained dBase-III/IV reader with TypeScript types, already used by PR 3b's migrate-usuario.ts). The `readTable` async iterable yields one row at a time with `legacyKey` resolved from the per-table PK column map (`TABLE_PK_COLUMN`). `readTableFromTable` is the test seam that adapts an in-memory `DataTable` to the same async-iterable shape.

- [x] TASK-053 — **PR 7 — packages/import — hash + pipeline** — `packages/import/src/hash.ts` (`computeHash(record): string` SHA-256 over canonicalized JSON), `packages/import/src/pipeline.ts` (`runImport({batchId, trigger, table?})` with ordered table iteration, `ON CONFLICT (source_table, source_key, content_hash) DO NOTHING` insert into `raw_events`, fail-fast on dependency violation). **Files (new)**: 2. **Lines ~200. **Deps: TASK-008, TASK-052. **AC**: re-running same batch with no changes inserts 0 new rows; changing 1 record inserts exactly 1 row. **Note**: Implemented in PR 7a (commit 59153bf, merged 597b94d). `runImport` uses a `basePath` (production) or `fixtures` (tests) source; order is `paramet → socios → ctacte → ctacte1 → asiento → asientod → cobros → plancue → catastros → escuela → deportes → locacion → caja → gastos` matching the spec's mandated dependency order. `TABLE_DEPENDENCIES` declares `ctacte1 → ctacte` and `asientod → asiento`; the pipeline aborts with `BusinessError(INTERNAL_ERROR)` if a child is imported before its parent. `raw_events` table added via migration `0006_great_shadow_king.sql` (uuid, source_table varchar(32), source_key varchar(64), content_hash varchar(64), payload jsonb, import_batch uuid, imported_at timestamptz) with unique index `(source_table, source_key, content_hash)` and B-tree index on `(source_table, source_key)`.

- [x] TASK-054 — **PR 7 — packages/import — bridge validator** — `packages/import/src/bridge-validator.ts` (`validateBridges(): Promise<OrphanAlert[]>` — checks CONNROASIE for orphan links by joining imported `socios` and `ctacte`, also checks 14-table dependency order). **Files (new)**: 1. **Lines ~100. **Deps: TASK-053. **AC**: fixture with 1 orphan returns 1 alert with `entity_id`; clean fixture returns `[]`. **Note**: Implemented in PR 7a (commit 59153bf, merged 597b94d). `validateBridges` returns `OrphanAlert[]` with 3 variants: `connroasie-missing-socio`, `connroasie-missing-roasie`, `dependency-missing`. CONNROASIE links are carried by `cobros` rows (the legacy `cobros.DBF` has `CONNROASIE_SOCIO` and `CONNROASIE_ROASIE` fields per row). The dependency-order check compares `max(imported_at)` per table; a child imported BEFORE its parent fires `dependency-missing`. Validator is pure (no audit_events writes — drift detector in PR 7b handles alert emission).
...
> **PR 7a complete (commit 59153bf, merged 597b94d). 32 new tests (290 → 322). Stack: stacked-to-main. Next: PR 7b (lineage + projection + drift + freshness + 3 stub job bodies).**
