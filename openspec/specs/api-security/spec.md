...
- [N16] All 12 gastos + gastos-ctacte routes enforce `requireRole('ADMIN')`; non-ADMIN returns 403
- [N16] Every 403 on a gastos route emits a `PERMISSION_DENIED` `audit_events` row with `operator_id`, `ip_address`, `endpoint`, `required_permission="ADMIN"`
