...
import {
  ...
  type CondonationLifecycle as Lifecycle,
...
export function CondonationDecisionDialog({
  request,
  operatorId,
  role,
  onRefresh,
  ...
  const [result, setResult] = useState<CondonationRequest | null>(null)
  ...
  const refreshLifecycle = async (requiredState: Lifecycle['state']) => {
    const page = await listCondonationLifecycle(request.snapshot.member_id)
  ...
  const apply = async (current: Lifecycle) => {
    if (!current.execution_id || current.execution_status === 'unavailable')
      throw new CondonationOperationError('partial_data')
    const currencies = new Set(request.snapshot.obligations.map((item) => item.currency))
    const amount = request.snapshot.obligations.reduce(
      (total, item) => total + item.outstanding_amount_cents,
      0,
    )
    if (currencies.size !== 1 || !Number.isSafeInteger(amount))
      throw new CondonationOperationError('partial_data')
    const descriptor = {
      operatorId,
      action: `condonation-application:${request.id}:${current.execution_id}`,
      draftFingerprint: current.execution_id,
    }
    const response = await executeCondonationRequest(
      request.id,
      current.execution_id,
      store.getOrCreate(descriptor),
    )
    if (
      response.approval_id.toLowerCase() !== request.id.toLowerCase() ||
      response.execution_id.toLowerCase() !== current.execution_id.toLowerCase() ||
      response.member_id.toLowerCase() !== request.snapshot.member_id.toLowerCase() ||
      response.currency !== [...currencies][0] ||
      response.approved_amount_cents !== amount
    )
      throw new CondonationOperationError('partial_data')
  ...
  const runApplication = async (current: Lifecycle) => {
    ...
    try {
      ...
      if (active.current)
        setError('Aprobada pendiente de aplicación. Reintentá la aplicación sin volver a aprobar.')
    } finally {
      busy.current = false
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const descriptor = {
      operatorId,
      action: `condonation-decision:${request.id}`,
    ...
    setError('')
    try {
      const response = await decideCondonationRequest(
        request.id,
        input,
        store.getOrCreate(descriptor),
      )
      ...
      if (kind === 'conflict') {
        ...
        setError(
          kind === 'permission'
            ...
            : 'No se pudo confirmar la decisión. Reintentá con los mismos datos para recuperar el resultado; no se ejecutó una condonación desde esta pantalla.',
        )
      }
    } finally {
      busy.current = false
      ...
      footer={
        <>
          ...
          {lifecycle?.state === 'approved_awaiting_execution' && !applied && (
            ...
            >
              {pending ? 'Aplicando condonación…' : 'Reintentar aplicación'}
            </button>
          )}
        </>
      }
    >
      <div className="space-y-4 [overflow-wrap:anywhere]">
        ...
        <p className="text-sm text-ink-700">
          Aprobar y aplicar mantiene dos controles del servidor, pero requiere una sola acción
          explícita.
        </p>
        {result || lifecycle ? (
          <>
            {lifecycle && (
              <CondonationLifecycle lifecycle={lifecycle} role={role} headingLevel={4} />
            )}
            <p
              role="status"
              tabIndex={-1}
          ...
          <>
            ...
            <CondonationLifecycle lifecycle={request} role={role} headingLevel={4} />
            {selfRequested ? (
              <p>Otro ADMIN o TESORERO debe decidir esta solicitud.</p>
            ) : (
