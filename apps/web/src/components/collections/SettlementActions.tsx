...
export function SettlementActions({ debt, shifts, onPayment, onReverse, headingLevel = 3 }: Props) {
  ...
  // prettier-ignore
  return <section aria-labelledby="settlement-actions-title" className="space-y-4 rounded-lg border p-4">{headingLevel === 3 ? <h3 id="settlement-actions-title" className="text-lg font-semibold">Acciones de pago</h3> : <h4 id="settlement-actions-title" className="text-lg font-semibold">Acciones de pago</h4>}{status&&<p ref={statusRef} role="status" aria-live="polite" tabIndex={-1}>{status}</p>}<button type="button" onClick={openAllocation} disabled={!eligible.length||!shifts.length}>Registrar pago</button>{!shifts.length&&eligible.length>0&&<p role="status">No hay turnos de caja abiertos para registrar el pago.</p>}{reversible.map((settlement)=><button key={settlement.id} type="button" onClick={(event)=>openReversal(settlement,event.currentTarget)}>Revertir liquidación {settlement.id}</button>)}
