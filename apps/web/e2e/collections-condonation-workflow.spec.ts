...
test(`condonation ${mode}: separate request, decision and execution`, async ({
  ...
  async function api(route: Route, actor: Actor) {
    ...
    if (method === 'GET') {
      ...
      // Collections loads the negotiated-agreement lineage of every obligation shown on the
      // debt panel (getObligationAgreements in apps/web/src/lib/api/dues.ts), independently
      // of which treatment is being exercised. Condonation never creates an agreement, so the
      // lineage stays empty. Without this route the request falls through to the
      // unexpected-request collector at the end of api() and fails the run.
      if (path === `/api/v1/dues/obligations/${obligationId}/agreements`)
        return send(route, 200, { active: null, revisions: [] })
    }
