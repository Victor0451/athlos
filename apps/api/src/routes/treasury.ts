...
// prettier-ignore
import { CashDeskService, type CloseCashCommand, type ExpenseCommand, type OpenCashCommand, type SupportingRecordCommand, type TenderCommand, type ReverseTenderCommand } from '../modules/dues/cash-desk.ts'
...
export const treasuryRoutes:FastifyPluginCallback<TreasuryRouteOptions>=(fastify,options,done)=>{const container:AppContainer=fastify.container,service=options.service??new CashDeskService(container.db)
  ...
  // P9 auto-open: idempotent period bootstrap for the unified cash module. Returns the
  // operator's own OPEN shift, creating it (last close remainder, last desk, or 0/front-desk
  // on a first-ever drawer) when none exists.
  fastify.post('/api/v1/treasury/shifts/ensure-open',SHIFT_OPEN_READ_GATE,async(request,reply)=>{gate(container);return reply.code(200).send({shift:dto(await service.ensureOpenShift!(context(request,key(request),{})))})})
  // prettier-ignore
  ...
  fastify.post<{Params:{id:string;tenderId:string}}>('/api/v1/treasury/shifts/:id/tenders/:tenderId/reverse',TENDER_GATE,async(request,reply)=>{gate(container);const params=throwIfInvalid(shiftTenderParams,request.params,'params'),body=throwIfInvalid(reverseBody,request.body??{},'body'),callerKey=key(request),input={...context(request,callerKey,body),shiftId:params.id,tenderId:params.tenderId,reason:body.reason??`Reversión del movimiento ${params.tenderId}`} as ReverseTenderCommand;return reply.code(201).send(await service.reverseTender!(input))})
  // prettier-ignore
