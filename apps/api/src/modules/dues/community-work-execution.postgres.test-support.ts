...
/**
 * Shared disposable-PostgreSQL harness for the community-work execution
 * suites (sequential proof + concurrency proof). Each test file that calls
 * `bootstrapCommunityWorkExecutionDb` gets its own randomly named database
 * (`athlos_cw_execution_<hex>`) on the ATHLOS_TEST_DATABASE_URL server, built
 * from the migration files and dropped by exact name in teardown. No peer
 * sweeping, no shared mutable state between files: vitest isolates module
 * instances per test file, so two suites boot two independent databases.
 */
...
export let approverId: string

export const count = async (query: string, values: unknown[] = []) =>
  Number((await db.pool.query(query, values)).rows[0].count)
export const obligationCounts = async (obligationId: string) => ({
...
export const executionContext = (callerKey = randomUUID()): AuditContext => ({
  actorId: approverId,
  role: 'ADMIN',
...
export const terms = (amountCents: number) => ({
  amountCents,
  installments: [{ amountCents, dueDate: '2026-12-01' }],
})

export async function fixtureObligation(amountCents = 10_000) {
...
export async function insertWorkToken(
...
export async function bootstrapCommunityWorkExecutionDb(): Promise<void> {
...
export async function teardownCommunityWorkExecutionDb(): Promise<void> {
