...
export class AssessmentService {
  ...
  private async plan(input: PreviewAssessmentCommand, db: DuesDb) {
    ...
    for (let period = input.fromPeriod; period <= input.throughPeriod;) {
      ...
      const benefits = await (this.repository.resolveBenefitRuleCandidates ?? repository.resolveBenefitRuleCandidates)(db, { socioId: input.socioId, familyGroupId: member.familyGroupId, period: { start, end } })
      ...
      periods.push({ period, start, end, calendarDays: plan.components[0]?.calendarDays ?? 0, components: [...plan.components.map((component) => ({ ...component, kind: component.componentKey === 'base' ? 'BASE' : 'SPORT', ...(component.componentKey === 'base' ? {} : { disciplinaId: member.enrollments.find((enrollment) => `sport:${enrollment.id}` === component.componentKey)?.disciplinaId }), status: existing ? 'ALREADY_GENERATED' : component.status })), ...(benefitResult?.components.map((component) => ({ ...component, eligibleFrom: start, eligibleTo: end, eligibleDays: plan.components[0]?.calendarDays ?? 0, calendarDays: plan.components[0]?.calendarDays ?? 0, segments: [], numerator: component.amountCents, remainder: 0, status: existing ? 'ALREADY_GENERATED' : 'PENDING' })) ?? [])], existingObligationId: existing?.id ?? null, pendingAmountCents: existing || !plan.executable ? 0 : benefitResult!.totalCents })
  ...
  async executeRange(input: ExecuteRangeCommand): Promise<RangeExecutionResult> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const period of plan.periods as Array<{ period: string; start: string; end: string; pendingAmountCents: number; components: Array<Record<string, unknown>> }>) {
        ...
        const components = period.components.map((component) => ({ kind: component.kind as 'BASE' | 'SPORT' | 'BENEFIT', componentKey: component.componentKey as string, amountCents: component.amountCents as number, priceVersionId: null, ...(typeof component.disciplinaId === 'string' ? { disciplinaId: component.disciplinaId } : {}), calculationInputs: component, eligibilitySnapshot: { eligibleFrom: component.eligibleFrom, eligibleTo: component.eligibleTo }, priceSnapshot: component.kind === 'BENEFIT' ? component.priceSnapshot as Json : { segments: component.segments } }))
