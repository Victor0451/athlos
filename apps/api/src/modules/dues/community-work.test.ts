...
it('records approved work as a non-cash settlement and explicit allocation', async () => {
  ...
  const result = await new CommunityWorkService(db(), { repository, audit }).create({ ...context, socioId: 'socio-1', obligationId: 'obligation-1', amountCents: 4_000, evidence: { approvalId: 'approval-1' }, reason: 'Approved work' })
  expect(result).toMatchObject({ settlementId: 'settlement-1', allocationId: 'allocation-1' })
...
it('replays an existing community-work claim without creating a second allocation', async () => {
  ...
  const result = await new CommunityWorkService(db(), { repository, audit: vi.fn() }).create({ ...context, socioId: 'socio-1', obligationId: 'obligation-1', amountCents: 4_000, evidence: { approvalId: 'approval-1' }, reason: 'Retry approved work' })
  expect(result).toMatchObject({ id: 'work-1', allocationId: 'allocation-1' })
