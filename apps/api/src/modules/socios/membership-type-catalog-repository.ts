...
type StateRow = { state: CatalogState }
...
export async function listMembershipTypeCatalog(
  ...
  if (!status || status.state !== 'ready')
    return { state: status?.state ?? 'no_current_catalog', items: [], total: 0, ...paging }
  ...
  const result = await rows<MembershipTypeCatalogItem & { total: number }>(
    ...
    WITH newest_catalog AS (
      ...
      SELECT c.source_row_id, c.code, r.name, r.letter FROM socios.legacy_membership_type_candidates c
    ...
    SELECT t.source_row_id AS "sourceRowId", t.code, t.name, t.letter,
