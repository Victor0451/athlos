...
describe('0007_entity_uuids migration', () => {
  it('migration file creates entity_uuids table with correct columns', async () => {
    ...
    // process.cwd() when running via pnpm --filter is /run/media/vlongo/Archivos/Projectos/Athlos/packages/db
    const migrationPath = path.join(process.cwd(), 'drizzle/0007_entity_uuids.sql')
