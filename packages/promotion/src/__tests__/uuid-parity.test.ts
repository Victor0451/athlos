 ...
 * to the local dev server at localhost). When the DB is unreachable
 ...
 * server (localhost) so local runs work without env config.
 */
function parseDatabaseUrl(): {
  ...
  const url = process.env.DATABASE_URL ?? 'postgresql://athlos:athlos@localhost:5432/athlos'
